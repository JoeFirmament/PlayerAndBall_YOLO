#!/usr/bin/env python3
"""
生成带白色边距的9x6棋盘格
"""

import os
from PIL import Image, ImageDraw

def generate_chessboard_with_margin():
    """生成带20mm边距的棋盘格"""
    print("生成带白色边距的9x6棋盘格...")

    # 参数设置
    board_corners = (9, 6)  # 9x6内角点
    square_size_mm = 25    # 25mm格子
    margin_mm = 20         # 20mm白色边距
    dpi = 300              # 300dpi

    # 计算尺寸
    squares_width = board_corners[0] + 1   # 10
    squares_height = board_corners[1] + 1  # 7
    board_width_mm = squares_width * square_size_mm   # 250mm
    board_height_mm = squares_height * square_size_mm # 175mm
    total_width_mm = board_width_mm + 2 * margin_mm   # 290mm
    total_height_mm = board_height_mm + 2 * margin_mm # 215mm

    # 转换为像素
    pixels_per_mm = dpi / 25.4
    board_width_px = int(board_width_mm * pixels_per_mm)
    board_height_px = int(board_height_mm * pixels_per_mm)
    margin_px = int(margin_mm * pixels_per_mm)
    square_size_px = int(square_size_mm * pixels_per_mm)

    total_width_px = board_width_px + 2 * margin_px
    total_height_px = board_height_px + 2 * margin_px

    print(f"原始棋盘: {board_width_mm}mm x {board_height_mm}mm")
    print(f"带边距总尺寸: {total_width_mm}mm x {total_height_mm}mm")
    print(f"像素尺寸: {total_width_px} x {total_height_px}")

    # 创建白色背景图像
    img = Image.new('RGB', (total_width_px, total_height_px), 'white')
    draw = ImageDraw.Draw(img)

    # 绘制棋盘格（带边距）
    board_x_start = margin_px
    board_y_start = margin_px

    for row in range(squares_height):
        for col in range(squares_width):
            x1 = board_x_start + col * square_size_px
            y1 = board_y_start + row * square_size_px
            x2 = x1 + square_size_px
            y2 = y1 + square_size_px

            # 黑白交替
            if (row + col) % 2 == 0:
                color = 'black'
            else:
                color = 'white'

            draw.rectangle([x1, y1, x2, y2], fill=color)

    # 添加红色尺寸线
    line_color = 'red'
    line_width = 2

    # 水平尺寸线
    draw.line([margin_px, margin_px//2, margin_px + board_width_px, margin_px//2],
             fill=line_color, width=line_width)

    # 垂直尺寸线
    draw.line([margin_px + board_width_px + margin_px//2, margin_px,
              margin_px + board_width_px + margin_px//2, margin_px + board_height_px],
             fill=line_color, width=line_width)

    # 保存文件
    filename = f"chessboard_9x6_{board_width_mm}x{board_height_mm}mm_margin{margin_mm}mm.png"
    filepath = os.path.join(os.getcwd(), filename)
    img.save(filepath, 'PNG', dpi=(dpi, dpi))

    print(f"✅ 棋盘格已生成: {filename}")
    print(f"📁 保存路径: {filepath}")

    # 文件信息
    file_size = os.path.getsize(filepath)
    print(".2f")

    return filepath

def show_print_guide():
    """显示打印指南"""
    print("\n🖨️ 打印指南:")
    print("1. 纸张: A2纸 (420x594mm)")
    print("2. 质量: 最高质量")
    print("3. 颜色: 彩色 (保留红色尺寸线)")
    print("4. 缩放: 100%")
    print("5. 摆放: 居中")

    print("\n✂️ 后期处理:")
    print("1. 保留20mm白色边距")
    print("2. 裁剪时留出至少5mm白边")
    print("3. 粘贴到硬质板材")
    print("4. 确保表面平整")

    print("\n🎯 优势:")
    print("• ✅ 白色边距便于固定")
    print("• ✅ 红色尺寸线便于对齐")
    print("• ✅ 专业外观")
    print("• ✅ 更好的检测效果")

if __name__ == "__main__":
    print("🀄️ 带白色边距的9x6棋盘格生成器")
    print("=" * 50)

    # 生成棋盘格
    filepath = generate_chessboard_with_margin()

    # 显示指南
    show_print_guide()

    print("\n🎉 生成完成！")
    print("现在可以将生成的图片打印到A2纸上")
