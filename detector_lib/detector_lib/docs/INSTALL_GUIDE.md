...
**好消息！** 从v1.0.1版本开始，所有必需的RKNN头文件已经内置在`detector_lib/include`中，**无需额外配置RKNN SDK路径**！
...
```bash
# 查找库文件
find /home -name "librknnrt.so*" 2>/dev/null

# 复制到系统库目录
sudo cp /path/to/librknnrt.so* /usr/lib/
sudo ldconfig
