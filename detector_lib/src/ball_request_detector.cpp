...
bool BallRequestDetector::detect_raw_request(const PoseResult& pose, float& confidence) const {
    ...
    // 检查要球条件
    bool hands_close = features.hands_distance_mm <= config_.max_hands_distance_mm;
    bool hands_in_chest_area = features.hands_to_chest_distance <= 
                              (config_.chest_region_scale * features.hands_distance_mm);
...
float BallRequestDetector::calculate_request_confidence(const GestureFeatures& features) const {
    ...
    if (features.hands_distance_mm > 0) {
        float ideal_distance = config_.max_hands_distance_mm * 0.7f;  // 理想距离为最大距离的70%
        float distance_diff = std::abs(features.hands_distance_mm - ideal_distance);
        distance_factor = std::max(0.0f, 1.0f - distance_diff / ideal_distance);
    }
    
    // 位置因子：手在胸前位置越合适置信度越高
    float position_factor = 1.0f;
    if (features.hands_to_chest_distance > 0) {
        float ideal_chest_distance = config_.max_hands_distance_mm * 0.5f;
        float chest_distance_diff = std::abs(features.hands_to_chest_distance - ideal_chest_distance);
        position_factor = std::max(0.0f, 1.0f - chest_distance_diff / ideal_chest_distance);
    }
