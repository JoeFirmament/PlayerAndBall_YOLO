...
int main(int argc, char* argv[]) {
    std::cout << "=== 姿态检测图片程序 v1.0.3 ===" << std::endl;
    
    // 1. 智能查找模型文件 (无需硬编码路径)
    std::string model_path = PathUtils::find_model("Q_yolov8_pose.rknn");
    if (model_path.empty()) {
        std::cerr << "❌ 无法找到模型文件 Q_yolov8_pose.rknn" << std::endl;
        std::cerr << "请检查以下路径:" << std::endl;
        std::cerr << "  - 环境变量 DETECTOR_MODEL_PATH" << std::endl;
        std::cerr << "  - ./models/, ../models/, ../../models/" << std::endl;
        std::cerr << "  - /usr/local/share/detector_lib/models/" << std::endl;
        return -1;
    }
    
    // 2. 处理图片路径 (支持命令行参数)
    std::string image_path = "../imgs/pose.jpg";  // 默认图片 (统一路径约定)
    if (argc >= 2) {
        image_path = argv[1];
    }
    
    std::cout << "✓ 模型文件: " << model_path << std::endl;
    std::cout << "✓ 图片文件: " << image_path << std::endl;
