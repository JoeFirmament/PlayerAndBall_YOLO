...
struct CalibrationPoint {
    ...
    CalibrationPoint(double px, double py, double wx, double wy)
        : pixel_x(px), pixel_y(py), world_x(wx), world_y(wy) {}
};
...
void demonstrate_homography_transformation() {
    ...
    calibration_points.push_back(CalibrationPoint(713.32, 484.66, 455.0, 5915.0));   // 标定点6
    calibration_points.push_back(CalibrationPoint(857.48, 613.90, 910.0, 2730.0));   // 标定点7
    calibration_points.push_back(CalibrationPoint(602.30, 531.06, -455.0, 4095.0));  // 标定点5
    calibration_points.push_back(CalibrationPoint(522.77, 550.94, -910.0, 3640.0));  // 标定点4

    for(size_t i = 0; i < calibration_points.size(); i++) {
        CalibrationPoint point = calibration_points[i];

        // 使用Homography变换
        std::pair<double, double> calculated_world = pixel_to_world(point.pixel_x, point.pixel_y);
        double calculated_world_x = calculated_world.first;
        double calculated_world_y = calculated_world.second;

        // 计算误差
        double error_x = std::abs(calculated_world_x - point.world_x);
        double error_y = std::abs(calculated_world_y - point.world_y);
        double error_distance = std::sqrt(error_x * error_x + error_y * error_y);

        std::cout << "\n标定点" << (i+4) << "：" << std::endl;
        std::cout << "  像素坐标：(" << point.pixel_x << ", " << point.pixel_y << ") px" << std::endl;
        std::cout << "  期望世界坐标：(" << point.world_x << ", " << point.world_y << ") mm" << std::endl;
    ...
    // 转换为世界坐标
    std::pair<double, double> athlete_head_world = pixel_to_world(athlete_head_pixel_x, athlete_head_pixel_y);
    double athlete_head_world_x = athlete_head_world.first;
    double athlete_head_world_y = athlete_head_world.second;

    std::pair<double, double> athlete_foot_world = pixel_to_world(athlete_foot_pixel_x, athlete_foot_pixel_y);
    double athlete_foot_world_x = athlete_foot_world.first;
    double athlete_foot_world_y = athlete_foot_world.second;
