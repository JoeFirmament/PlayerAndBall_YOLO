...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./yolov8_pose_labels_list.txt"
...
int pose_post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold,
...
int pose_init_post_process() {
    // 硬编码标签，避免文件路径问题
    labels[0] = (char*)"player";
    printf("✓ 硬编码标签初始化成功\n");
    return 0;
}

char *pose_coco_cls_to_name(int cls_id) {
