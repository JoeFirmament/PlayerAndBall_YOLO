...
// 篮球检测线程相关全局变量
// ----------------------
// 线程安全队列，用于传递 undistorted/resize 后的图像
...
static int init_camera_mapping() {
    ...
    char calib_path[512];
    char homo_path[512];
    snprintf(calib_path, sizeof(calib_path), "%sdata/camera_calibration.xml", exe_path);
    snprintf(homo_path, sizeof(homo_path), "%sdata/homography.xml", exe_path);
...
void basketball_detect_thread_func() {
    printf("[basketball] 篮球检测线程已启动\n");
    // 1. 加载篮球检测模型
    const char* basketball_model_path = "/home/radxa/Qworkspace/yolov8_post_basketball/models/basketball_player_rk3588.rknn";
    rknn_app_context_t basketball_ctx;
    memset(&basketball_ctx, 0, sizeof(rknn_app_context_t));
    int ret = 0;
    // 设置NPU核心为NPU2，确保与主线程分开运行
    ...
    if (ret != 0) {
        ...
        printf("[basketball] 已成功将模型绑定到 NPU2 (core_mask=2)\n");
    }
    ret = init_yolov8_pose_model(basketball_model_path, &basketball_ctx); // 当前接口只支持2参数
    if (ret != 0) {
        printf("[basketball][error] 模型加载失败! 路径: %s\n", basketball_model_path);
        return;
    }
    printf("[basketball] 模型加载成功, 输入尺寸: %dx%d\n", basketball_ctx.model_width, basketball_ctx.model_height);
    ...
    while (basketball_thread_running) {
        ...
        printf("[basketball] 收到一帧用于篮球检测, 尺寸: %dx%d, 时间戳: %ld\n", input_img.cols, input_img.rows, getCurrentTimeUs());
        // 2.1 resize 到模型输入尺寸
        cv::Mat resized_img;
        cv::resize(input_img, resized_img, cv::Size(basketball_ctx.model_width, basketball_ctx.model_height));
        // 2.2 预处理（BGR->RGB，归一化等，视模型要求而定）
        cv::Mat rgb_img;
        ...
        // 2.4 推理
        int64_t t_infer_start = getCurrentTimeUs();
        ret = rknn_run(basketball_ctx.rknn_ctx, NULL);
        int64_t t_infer_end = getCurrentTimeUs();
        if (ret < 0) {
        ...
        output.index = 0;
        output.want_float = 1;
        ret = rknn_outputs_get(basketball_ctx.rknn_ctx, 1, &output, NULL);
        if (ret < 0) {
            printf("[basketball][error] rknn_outputs_get 失败! ret=%d\n", ret);
            continue;
        }
        // 2.6 解析输出，查找 class_id==1 的框
        float* out_ptr = (float*)output.buf;
        int num_boxes = 8400;
        int num_attrs = 6;
        int basketball_count = 0;
        for (int i = 0; i < num_boxes; ++i) {
            float x = out_ptr[i * num_attrs + 0];
            float y = out_ptr[i * num_attrs + 1];
            float w = out_ptr[i * num_attrs + 2];
            float h = out_ptr[i * num_attrs + 3];
            float obj_conf = out_ptr[i * num_attrs + 4];
            float class_id = out_ptr[i * num_attrs + 5];
            if ((int)class_id == 1 && obj_conf > 0.5) { // 只输出篮球
                basketball_count++;
                printf("[basketball][detect] 检测到篮球: box=(%.1f,%.1f,%.1f,%.1f), conf=%.2f\n", x, y, w, h, obj_conf);
                // 推送检测结果到队列
                BasketballDetectResult result;
                result.x = x;
                result.y = y;
                result.w = w;
                result.h = h;
                result.conf = obj_conf;
                result.timestamp = getCurrentTimeUs();
                {
                    std::lock_guard<std::mutex> lock(basketball_result_mutex);
                    basketball_result_queue.push(result);
                    // 限制队列长度，防止堆积
                    while (basketball_result_queue.size() > 5) basketball_result_queue.pop();
                }
                basketball_result_cv.notify_one();
                printf("[basketball][queue] 已推送检测结果到主线程, 当前队列长度: %zu\n", basketball_result_queue.size());
            }
        }
        if (basketball_count == 0) {
            printf("[basketball][detect] 本帧未检测到篮球\n");
        }
        printf("[basketball] 推理耗时: %.2f ms\n", (t_infer_end-t_infer_start)/1000.0);
        rknn_outputs_release(basketball_ctx.rknn_ctx, 1, &output);
    }
    // 3. 释放模型
    release_yolov8_pose_model(&basketball_ctx);
    printf("[basketball] 篮球检测线程已退出\n");
}
    ...
    while (g_running) {
        ...
        // --- 取出篮球检测结果并画框 ---
        {
            ...
            if (!basketball_result_queue.empty()) {
                ...
                // 将模型输入尺寸下的框映射到 undistorted_roi 尺寸
                float scale_x = (float)undistorted_roi.cols / (float)zero_copy_ctx.model_width;
                float scale_y = (float)undistorted_roi.rows / (float)zero_copy_ctx.model_height;
                float cx = result.x * scale_x;
                float cy = result.y * scale_y;
                float w = result.w * scale_x;
                float h = result.h * scale_y;
                cv::Rect bbox((int)(cx - w/2), (int)(cy - h/2), (int)w, (int)h);
                cv::rectangle(result_frame, bbox, cv::Scalar(0,0,255), 2);
                char conf_text[64];
                sprintf(conf_text, "Basketball: %.2f", result.conf);
                cv::putText(result_frame, conf_text, cv::Point(bbox.x, bbox.y-10), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0,0,255), 2);
                printf("[main][draw] 已在画面上绘制篮球检测框: (%d,%d,%d,%d), conf=%.2f\n", bbox.x, bbox.y, bbox.width, bbox.height, result.conf);
            }
        }
        ...
        // --- 在此处将 undistorted_roi 送入篮球检测队列 ---
        {
            std::lock_guard<std::mutex> lock(basketball_queue_mutex);
            // 限制队列长度，防止内存暴涨
            if (basketball_frame_queue.size() < 3) {
                basketball_frame_queue.push(undistorted_roi.clone());
                basketball_queue_cv.notify_one();
                printf("[main] 已送入一帧到篮球检测队列, 当前队列长度: %zu\n", basketball_frame_queue.size());
            } else {
                printf("[main] 篮球检测队列已满，丢弃一帧\n");
            }
        }
    }
