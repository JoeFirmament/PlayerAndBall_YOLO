...
// 支持NPU核心选择的初始化函数
int init_yolov8_pose_model_with_npu(const char *model_path, rknn_app_context_t *app_ctx, int npu_core_id)
{
    int ret;
    rknn_context ctx = 0;
    ...
    if (ret < 0) {
        printf("⚠️ 设置姿态检测NPU核心%d失败, 使用默认设置\n", npu_core_id);
    } else {
        printf("✅ 姿态检测模型使用NPU核心%d\n", npu_core_id);
    }
    ...
    printf("model is %s: input %dx%dx%d\n", 
           (input_attrs[0].fmt == RKNN_TENSOR_NCHW) ? "NCHW" : "NHWC",
           app_ctx->model_channel, app_ctx->model_height, app_ctx->model_width);

    app_ctx->is_quant = (input_attrs[0].type != RKNN_TENSOR_FLOAT32);

    return 0;
}

// 零拷贝推理专用的输出处理函数
int process_yolov8_pose_outputs(rknn_app_context_t* app_ctx, void* zero_copy_ctx, object_detect_result_list* od_results)
{
    // 这里应该实现零拷贝的输出处理
    // 目前先简化处理，直接返回空结果
    od_results->count = 0;
